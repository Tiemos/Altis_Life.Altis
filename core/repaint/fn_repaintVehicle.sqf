...
if ((getPlayerUID player) != _vehOwner) exitWith {hintSilent "You aren't the owner!";_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};
		...
		if(player != vehicle player) exitWith {titleText["You have to get out of the vehicle in order to paint it can!","PLAIN"];_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};
