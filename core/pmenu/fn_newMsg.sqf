...
switch(_type) do
{
	...
	case 1:
	{
		if(isNUll life_smartphoneTarget) exitWith {hintSilent format["No person selected!"];};
	...
	case 2:
	{
		if(({side _x == west} count playableUnits) == 0) exitWith {hintSilent format["The police is currently not accessible. Please try again later."];};
	...
	case 4:
	{
		if(({side _x == independent} count playableUnits) == 0) exitWith {hintSilent format["Currently there is no doctor on duty. Please try it again later."];};
