...
switch (true) do
{
	...
	{
		if(vehicle player != player) exitWith {hintSilent "You can not heal in a vehicle ..."};
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			player setDamage 0;
			player setFatigue 0;
			...
			closeDialog 0;
			hintSilent "The painkillers have worked! You now again full of life."
		};
	};
	...
	case (_item == "battery"):
	{
		...
		{
			life_battery = 100;
			hintSilent "My battery is now at 100%.";
		};
	};
	...
	{
		if(!isNull life_spikestrip) exitWith {hintSilent "You've already nail belt in his hand."};
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
