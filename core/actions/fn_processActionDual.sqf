...
if ((_type == "uranium2") && (life_inv_puranium == 0)) exitWith {hintSilent "You have no product to dissolve the uranium.";};
...
if(_hasLicense) then
{
    ...
    5 cutText ["","PLAIN"];
    titleText[format["They have %1 %2",_oldVal,_itemName],"PLAIN"];
    life_is_processing = false;
}
    else
{

    if(life_cash < _cost) exitWith {hintSilent format["You need $%1 for a license",[_cost] call life_fnc_numberText]; 5 cutText ["","PLAIN"]; life_is_processing = false;};
    ...
    if(player distance _vendor > 10) exitWith {hintSilent "You have to stand by at least 10 meters."; 5 cutText ["","PLAIN"]; life_is_processing = false;};
    ...
    if(!([true,_newItem,_oldVal] call life_fnc_handleInv)) exitWith {5 cutText ["","PLAIN"]; [true,_oldItem1,_oldVal] call life_fnc_handleInv; [true,_oldItem2,_oldVal] call life_fnc_handleInv; life_is_processing = false;};
    5 cutText ["","PLAIN"];
    titleText[format["You have %1 converted to %2 and receive $3%",_oldVal,_itemName,[_cost] call life_fnc_numberText],"PLAIN"];
